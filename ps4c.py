...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    """
    ciphertext_str = ps4b.EncryptedMessage.get_text(ciphertext)
    word_freq = {}
    enc_msg_tp = ps4b.EncryptedMessage(ciphertext_str)
    word_list = load_words("words.txt")
    for i in range(len(pads)):
        word_freq[tuple(pads[i])] = 0
    for y in pads:
        ptm = enc_msg_tp.decrypt_message(y)
        ptm_split = ptm.get_text().split(" ")
        for word in ptm_split:
            if is_word(word_list, word):
                word_freq[tuple(y)] += 1
    correct_pad = tuple(pads[len(pads) - 1])
    for key in word_freq:
        if word_freq[key] >= word_freq[correct_pad]:
            correct_pad = key
    result = ps4b.PlaintextMessage(
        enc_msg_tp.decrypt_message(correct_pad).get_text(), list(correct_pad)
    )
    return result
